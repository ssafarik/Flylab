...
class CalibrateStageArena():
    def __init__(self):
        ...
        self.start_points_projected  = cv.CreateMat(self.nGridlines, 2, cv.CV_32FC1)
        self.end_points_projected    = cv.CreateMat(self.nGridlines, 2, cv.CV_32FC1)
    ...
    def InitializeImages(self,cv_image):
        ...
        # self.im = cv.CreateImage((self.im_width, self.im_height),cv.IPL_DEPTH_8U,1)
        self.im_display = cv.CreateImage((self.im_width, self.im_height),cv.IPL_DEPTH_8U,3)
    ...
    def CameraInfo_callback (self, msgCameraInfo):
        if self.camerainfo is None:
            ...
            cv.SetReal2D(self.tvec, 0, 0, 0.0)
            cv.SetReal2D(self.tvec, 0, 1, 0.0)
            cv.SetReal2D(self.tvec, 0, 2, 0.0)
    ...
    def DrawOriginAxes(self, cvimage):
        if self.camerainfo is not None:
            ...
            try:
                ...
                # draw x-axis
                pt2 = tuple(a[1])
                cv.Line(cvimage, pt1, pt2, cv.CV_RGB(self.color_max,0,0), axis_line_width)
                
                # draw y-axis
                pt2 = tuple(a[2])
                cv.Line(cvimage, pt1, pt2, cv.CV_RGB(0,self.color_max,0), axis_line_width)
                
                # draw z-axis
                pt2 = tuple(a[3])
                cv.Line(cvimage, pt1, pt2, cv.CV_RGB(0,0,self.color_max), axis_line_width)
    ...
    def DrawGrid(self, cvimage):
        if self.camerainfo is not None:
            points_range = (N.array(range(self.nGridlines)) - (self.nGridlines-1)/2) * self.checker_size
            points_zeros = N.zeros(points_range.shape)
            points_ones = N.ones(points_range.shape) * self.checker_size * (self.nGridlines - 1)/2
            ...
            for line_n in range(self.nGridlines):
                cv.Line(cvimage, tuple(start_points[line_n,:]), tuple(end_points[line_n,:]), cv.CV_RGB(self.color_max,0,0), axis_line_width)
            ...
            for line_n in range(self.nGridlines):
                cv.Line(cvimage, tuple(start_points[line_n,:]), tuple(end_points[line_n,:]), cv.CV_RGB(0,self.color_max,0), axis_line_width)
    ...
    def Image_callback(self, image):
        if self.initialized:
            ...
            if self.initialized_pose:
                ...
                # cv.PutText(self.im_display,display_text,(xText,yText),self.font,self.font_color)
                #yText += dyText
                ...
                if self.point_count_min < self.arena_point_array.shape[1]:
                    ...
                    self.DrawGrid(self.im_display)
                
                    self.DrawOriginAxes(self.im_display)
                    self.DrawGrid(self.im_display)
                    
        
            
            cv.PutText(self.im_display, self.error_text, (xText,yText), self.font, self.font_color)
    ...
    def ContourinfoLists_callback(self,contourinfo_lists):
        if self.initialized:
            ...
            if contour_count==1:
                ...
                self.error_text = 'ERROR:  No objects detected.'
